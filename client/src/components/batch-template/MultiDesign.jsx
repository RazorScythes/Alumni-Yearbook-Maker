...
const MultiDesign = () => {
    ...
    const handleTemplate2 = (e) => {
        if(e.index) {
            setIndex({...index, template2: e.index});
            setId({...id, template2: templateList[e.index]._id});
        }
    };

    const handleTemplate3 = (e) => {
        if(e.index) {
            setIndex({...index, template3: e.index});
            setId({...id, template3: templateList[e.index]._id});
        }
    };

    const handleTemplate4 = (e) => {
        if(e.index) {
            setIndex({...index, template4: e.index});
            setId({...id, template4: templateList[e.index]._id});
        }
    };
    ...
    const handleSubmit = async (e) => {
        if((templateList === undefined) && (coverList === undefined) && (nametagsList === undefined)) return
        if(!dataUri){
            setRequire(true)
            return
        }
        ...
        /*   Template   */
        if(!id.template1) t.template1 = templateList[0]._id
        else t.template1 = id.template1
        if(!id.template2) t.template2 = templateList[0]._id
        else t.template2 = id.template2
        if(!id.template3) t.template3 = templateList[0]._id
        else t.template3 = id.template3
        if(!id.template4) t.template4 = templateList[0]._id
        else t.template4 = id.template4
        ...
        setForm(
            {
                ...
                closing_song: {
                    song_title: e.song_title ? e.song_title : '',
                    singer: e.singer ? e.singer : '',
                    lyrics: e.lyrics ? e.lyrics : ''
                },
        ...
        <>     
                ...
                {formik => (
                    ...
                    <Row style={{marginBottom:5}}>
                        ...
                        <div style={{width:"100%", display:"inline-block"}}>
                            ...
                            <button className="btn btn-dark mt-3" style={{float:"right", marginRight:10}} disabled={(templateList !== undefined && templateList.length < 1) || (coverList !== undefined && coverList.length < 1) || (nametagsList !== undefined && nametagsList.length < 1)} type="submit">Register</button>
                        </div>
                    </Form>
                    </div>
                )}
...

            {
                ...
                <>
                    ...
                    <Container fluid>
                        <Row>
                            <Col><h3>Select Pages Template</h3></Col>
                        </Row>
                    </Container>
                    ...
                    <Container fluid>
                        <Row>
                            <Col><h3 style={{margin:"20px 0"}}>Select Cover Page and Nametag</h3></Col>
                        </Row>
                    </Container>
                    <div className="flexbox-2">
                        {
